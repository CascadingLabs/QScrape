...
{:else if view === 'detail' && currentArticle}
  <div data-component="news-article-detail" data-article-id={currentArticle.id} class="hn-detail">
    <button type="button" class="hn-back-btn" on:click={() => history.back()}>← Back to articles</button>
    <div data-category={currentArticle.category} class="hn-detail-cat">{currentArticle.category}</div>
    {#if currentArticle.breaking}
      <span class="hn-breaking-tag">BREAKING</span>
    {/if}
    ...
    {#if currentArticle.tags.length}
      <div class="hn-detail-tags">
        {#each currentArticle.tags as tag (tag)}
          <span class="hn-tag">{tag}</span>
        {/each}
      </div>
    {/if}
  </div>
{:else}
  <div data-component="news-article-feed" data-framework="svelte">
    <div class="hn-feed-count">{allFiltered.length} article{allFiltered.length !== 1 ? 's' : ''}{cat ? ` in ${cat}` : ''}</div>
    <div class="hn-feed-list">
      {#each items as a (a.id)}
        <article data-article-id={a.id} data-category={a.category} class="hn-feed-item" on:click={() => goToArticle(a.id)} role="button" tabindex="0" on:keydown={(e) => e.key === 'Enter' && goToArticle(a.id)}>
