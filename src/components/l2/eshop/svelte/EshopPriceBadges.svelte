...
{:else}
  <div data-component="eshop-price-badges" data-framework="svelte" class="vm-pbadge">
    <h3 class="vm-pbadge-title">Sale Prices</h3>
    <ul class="vm-pbadge-list">
      {#each saleItems as p (p.sku)}
        <li data-sku={p.sku} class="vm-pbadge-item" on:click={() => goToProduct(p.sku)} role="button" tabindex="0" on:keydown={(e) => e.key === 'Enter' && goToProduct(p.sku)}>
