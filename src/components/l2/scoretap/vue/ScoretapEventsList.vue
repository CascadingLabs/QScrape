...
const _selectedEvent = computed(() =>
	view.value === 'event' && selectedId.value
		? (events.find((e) => e.id === selectedId.value) ?? null)
		: null,
);
...
<template>
  ...
  >
    ...
    <div class="st-detail-body">
      ...
      <div class="st-detail-meta">
        <span
          class="st-game-tag"
          :style="{ background: _gameColors[selectedEvent.game] }"
          :data-game="selectedEvent.game"
        >{{ _gameLabels[selectedEvent.game] }}</span>
        <span
          class="st-status-badge"
          :class="{ 'st-status-live': selectedEvent.status === 'live' }"
        >
  ...
  >
    ...
    <div v-if="filtered.length === 0" class="st-empty">No events.</div>
    <div
