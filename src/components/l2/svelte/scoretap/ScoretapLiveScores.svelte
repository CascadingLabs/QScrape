...
<script lang="ts">
  ...
  let scores = Object.fromEntries(liveMatches.map((m) => [m.id, { a: m.scoreA, b: m.scoreB }]));
  ...
  function filterArr<T extends { game: Game }>(arr: T[]): T[] {
    return activeGame === 'all' ? arr : arr.filter((x) => x.game === activeGame);
  }

  $: filteredLive = filterArr(liveMatches);
  $: filteredResults = filterArr(recentResults);
  $: filteredNews = filterArr(newsItems).slice(0, 6);
  $: matchData = selectedId ? (allMatches.find((x) => x.id === selectedId) ?? null) : null;
  $: articleData = selectedId ? (newsItems.find((x) => x.id === selectedId) ?? null) : null;
  $: teamData = selectedId ? (teams.find((x) => x.id === selectedId) ?? null) : null;
  $: eventData = selectedId ? (events.find((x) => x.id === selectedId) ?? null) : null;
  ...
  function onMatch(e: Event) { view = 'match'; selectedId = (e as CustomEvent<string>).detail; }
  function onArticle(e: Event) { view = 'article'; selectedId = (e as CustomEvent<string>).detail; }
  function onTeam(e: Event) { view = 'team'; selectedId = (e as CustomEvent<string>).detail; }
  function onEvent(e: Event) { view = 'event'; selectedId = (e as CustomEvent<string>).detail; }
  ...
  onMount(() => {
    ...
    ticker = setInterval(() => {
      ...
      const cur = scores[key][side as 'a' | 'b'];
      if (cur < cap) {
        scores = { ...scores, [key]: { ...scores[key], [side]: cur + 1 } };
      }
    }, 8000);
  });
  onDestroy(() => {
...
{:else if view === 'match' && selectedId && matchData}
  <div data-component="scoretap-match-detail" data-match-id={matchData.id} class="st-detail-wrap">
    <button type="button" class="st-back-btn" on:click={() => history.back()}>← Back to scores</button>
    <div class="st-widget">
      <div class="st-detail-header">
        ...
        <span class="st-status-badge" class:st-status-live={matchData.status === 'live'}>
      ...
      <div class="st-match-vs">
        ...
        <div class="st-vs-score">
          ...
          {#if matchData.mapOrGame}<div class="st-vs-map">{matchData.mapOrGame}</div>{/if}
        </div>
...
{:else if view === 'article' && selectedId && articleData}
  <div data-component="scoretap-article-detail" data-article-id={articleData.id} class="st-detail-wrap">
    <button type="button" class="st-back-btn" on:click={() => history.back()}>← Back to news</button>
...
{:else if view === 'team' && selectedId && teamData}
  <div data-component="scoretap-team-detail" data-team-id={teamData.id} class="st-detail-wrap">
    <button type="button" class="st-back-btn" on:click={() => history.back()}>← Back to scores</button>
    <div class="st-widget st-detail-body">
      ...
      {#if teamData.rank}
        <div class="st-rank-stats">
          ...
          {/if}
          {#if teamData.rankChange}
            <div class="st-stat-box">
              <div class="st-stat-val" class:st-up={teamData.rankChange === 'up'} class:st-down={teamData.rankChange === 'down'}>
...
{:else if view === 'event' && selectedId && eventData}
  <div data-component="scoretap-event-detail" data-event-id={eventData.id} class="st-detail-wrap">
    <button type="button" class="st-back-btn" on:click={() => history.back()}>← Back to events</button>
    <div class="st-widget st-detail-body">
      <div class="st-detail-meta">
        <span class="st-game-tag" style="background:{gameColors[eventData.game]}">{gameLabels[eventData.game]}</span>
        <span class="st-status-badge" class:st-status-live={eventData.status === 'live'}>
...
{:else}
  <div data-component="scoretap-live-scores" data-framework="svelte">

    <!-- Live scores -->
    <div class="st-widget">
      <div class="st-widget-head">Live Scores</div>
      {#each filteredLive as m (m.id)}
        <div data-match-id={m.id} data-game={m.game} class="st-match-row st-match-live st-clickable" on:click={() => goTo('match', m.id)}>
    ...
    <div class="st-widget">
      ...
      {#each filteredResults as m (m.id)}
        <div data-match-id={m.id} data-game={m.game} class="st-match-row st-clickable" on:click={() => goTo('match', m.id)}>
          <div class="st-team st-team-right">
            <span class="st-team-name" class:st-winner={m.scoreA > m.scoreB}>{m.teamA}</span>
          </div>
          <div class="st-match-center">
          ...
          <div class="st-team">
            <span class="st-team-name" class:st-winner={m.scoreB > m.scoreA}>{m.teamB}</span>
          </div>
        </div>
      {/each}
    </div>
    ...
    <div class="st-widget">
      ...
      <div class="st-news-list">
        {#each filteredNews as item (item.id)}
          <div data-game={item.game} class="st-news-item st-clickable" on:click={() => goTo('article', item.id)}>
